...
| Pipeline de video (Remotion) | 🟢 VERDE | Renderiza, y ya no puede renderizar contenido publicable sin origen verificable. Desde la auditoría de arte: zona segura, paleta y cuerpos mínimos leídos de la política visual (`src/brandTypography.ts`), y contraste del texto **medido** sobre el fotograma real (`scripts/audit-video-legibility.py`, en el workflow de render). |
...
| Carrusel (reparto en páginas) | 🟢 VERDE (nuevo) | `composition.build_carousel_plan()` + `compositor.compose_carousel()`: reparte el texto aprobado en páginas del mismo documento, solo por frontera de frase, con invariante de que la unión devuelve el texto palabra por palabra. Folio determinista, autor en la última página, aviso si se repite la imagen. Desbloquea PIEZA-01. El ensamblado del documento final sigue siendo humano (Canva), como dice la skill. |
| Composición editorial | 🟢 VERDE (aprobado) | `tipografia.ornamentos` (política 1.3): comilla colgante en el margen, filete de latón, versalitas con tracking y anclaje del texto decidido midiendo la imagen bloque a bloque. Fijado por prueba que los adornos **no mueven el texto**. Amarillo porque son decisiones de diseño nuevas, no transcripción de la skill: esperan confirmación del fundador. Ver `docs/diseno-editorial-v1.md`. |

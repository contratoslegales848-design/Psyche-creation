...
**Importante (cerrado en la Fase 1D):** que el hostname sea oficial tampoco basta para que esa fuente respalde **cualquier** país. La tabla cerrada `OFFICIAL_HOSTNAME_JURISDICTIONS` fija qué país(es) puede respaldar legítimamente cada hostname oficial conocido (p. ej. `boe.es` solo España; `diputados.gob.mx` solo México) — una fuente oficial nacional que declare en `jurisdicciones_cubiertas` un país ajeno a su propio organismo es rechazada con un error específico, sin importar qué tan convincente parezca el resto del JSON. Las fuentes supranacionales (`eur-lex.europa.eu`) tienen un ámbito explícito propio ("Unión Europea"), nunca un país concreto asumido. Solo las fuentes académicas/secundarias (Nivel 3, techo `APTO_CON_MATICES`) pueden declarar cobertura de varios países sin esta restricción — precisamente porque nunca pueden sostener por sí solas `APTO_PARA_NARRATIVA`.

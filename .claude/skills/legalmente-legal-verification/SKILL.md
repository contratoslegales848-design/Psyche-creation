...
**En todo claim multijurisdiccional, incluido `CAPA_B_VARIABLE`, el techo jurídico se calcula como el mínimo de cobertura entre todas las jurisdicciones declaradas en `jurisdiccion`** — nunca el máximo de cualquier fuente suelta (Fase 1E). Antes, un claim `CAPA_B_VARIABLE` con `jurisdiccion: ["México", "España", "Argentina"]` podía declararse `APTO_PARA_NARRATIVA` con una sola fuente Nivel 1 de cualquiera de los tres países, aunque los otros dos no tuvieran cobertura equivalente — esa fuente elevaba todo el claim. Esa misma protección de "mínimo por país" que ya tenía Capa C (y Capa A, con `jurisdicciones_revisadas`) ahora se aplica también a Capa B: cada país que aparece en `jurisdiccion` necesita su propia fuente Nivel 1 (o el claim queda topado al nivel del país más débil), y una fuente sin `registro_oficial_id` coherente nunca alcanza Nivel 1 por sí sola, sin importar cuántos booleanos de `verificacion_fuente` estén en `true`.
...
**Ninguno de esos tres booleanos, ni siquiera los tres juntos, basta para que una fuente oficial alcance Nivel 1.** Además hace falta declarar `registro_oficial_id`: el identificador de la entrada correspondiente en el **registro oficial único** (`references/official-source-registry.json`, Fase 1D.1). Ese registro — y solo ese, sin listas paralelas — decide, para cada organismo conocido: qué hostname(s) puede usar, su nombre canónico y alias aceptados, qué `tipo_fuente` puede emitir (p. ej. el BOE solo `NORMA_OFICIAL`, nunca `JURISPRUDENCIA_OFICIAL` — eso es el Poder Judicial), y qué jurisdicción/ámbito puede respaldar. El validador cruza los cuatro: hostname real de la URL (coincidencia exacta o de subdominio real, nunca por subcadena — `boe.es.evil.com` y `notboe.es` NO cuentan como `boe.es`; cuando varias entradas coinciden por subdominio, gana la más específica, p. ej. `dof.gob.mx` sobre la entrada genérica `gob.mx`), `organismo_autor` (comparación exacta tras normalizar, nunca por subcadena), `tipo_fuente` permitido, y `jurisdicciones_cubiertas` como subconjunto de lo autorizado. Cualquier incoherencia entre estos cuatro es `[ERROR ESTRUCTURAL]`, no una advertencia — declarar `registro_oficial_id="boe-es"` con `organismo_autor: "Suprema Corte de Justicia de México"` se rechaza aunque el hostname sea legítimamente `boe.es`. Un hostname/organismo legítimo que falte en el registro no falla con error duro por sí solo (genera advertencia) pero tampoco puede alcanzar Nivel 1 hasta que un humano añada la entrada. **Sin `url` real, tampoco hay Nivel 1** (Fase 1D.2): una fuente oficial con solo `identificador_bibliografico` puede ser estructuralmente válida, pero como máximo Nivel 2/`APTO_CON_MATICES` — no hay ningún hostname que verificar, así que `registro_oficial_id`/`organismo_autor`/los tres booleanos son autoafirmaciones sin evidencia de dominio, por coherentes que parezcan entre sí. La propia URL se valida con una única función canónica que rechaza barra invertida, userinfo, espacios/caracteres de control y puertos inválidos — una barra invertida en la URL es en sí misma motivo de rechazo, nunca algo que el validador intente "normalizar" para adivinar qué host quisiste decir. Si no pudiste acceder al texto oficial (p. ej. `WebFetch` bloqueado en el entorno), dilo en `observaciones` y deja `texto_exacto_consultado: false` — esa fuente entonces no puede sostener `APTO_PARA_NARRATIVA`, como máximo `APTO_CON_MATICES`.
...
### Etapa 6 — Emitir la pieza (JSON, esquema v4)
